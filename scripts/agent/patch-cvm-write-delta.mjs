...
const registryPath = 'backend/src/modules/capital-markets/cvmDatasetRegistry.ts';
let registry = fs.readFileSync(registryPath, 'utf8');
const registryAnchor = `    source_code: string;
    record_key: string;
    event_type: string;`;
if (!registry.includes(registryAnchor)) throw new Error('Registry content_hash anchor not found');
registry = registry.replace(registryAnchor, `    source_code: string;
    record_key: string;
    content_hash: string;
    event_type: string;`);
...
if (!connector.includes(identityBefore)) throw new Error('Connector stable identity anchor not found');
...
if (!connector.includes(eventBefore)) throw new Error('Connector event hash anchor not found');
connector = connector.replace(eventBefore, `    event: {
      dataset_code: input.datasetCode,
      source_code: definition.sourceCode,
      record_key: recordKey,
