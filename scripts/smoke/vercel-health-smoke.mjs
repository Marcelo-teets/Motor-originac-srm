...
function assertPayload(payload) {
  ...
  if (!payload.env?.resolvedUseSupabase) throw new Error('Supabase runtime is not resolved as active.');
  if (!payload.env?.SUPABASE_URL) throw new Error('SUPABASE_URL is not visible to runtime.');
  if (!payload.env?.SUPABASE_ANON_KEY) throw new Error('SUPABASE_ANON_KEY is not visible to runtime.');
  if (!payload.env?.SUPABASE_SERVICE_ROLE_KEY) throw new Error('SUPABASE_SERVICE_ROLE_KEY is not visible to runtime.');
  if (!payload.env?.CRON_SECRET) throw new Error('CRON_SECRET is not visible to runtime.');
  if (!payload.captureRuntime?.canRunAgainstSupabase) throw new Error('Capture runtime cannot run against Supabase.');
  if (!payload.captureRuntime?.canAuthorizeWorkflow) throw new Error('Capture workflow authorization is not configured.');
