...
export function CompanyDecisionBriefPanel({ companyId }: CompanyDecisionBriefPanelProps) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    void load().catch((loadError) => active && setError(loadError instanceof Error ? loadError.message : 'Falha ao preparar briefing.')).finally(() => active && setLoading(false));
    return () => { active = false; };
  }, [load]);

  const evidenceCount = useMemo(() => workspace
    ? workspace.signals.length + workspace.monitoringOutputs.length + workspace.nodes.reduce((sum, node) => sum + (node.referenceCount ?? 0), 0)
    : 0, [workspace]);
  ...
  const saveBrief = async () => {
    ...
    try {
      ...
      const saved = await knowledgeVaultApi.saveNode(session, {
        ...
        properties: {
          template: 'company-decision-brief-v12', generatedAt: new Date().toISOString(), humanConfirmed: true,
          evidenceCount, qualificationSnapshotId: workspace.latestQualification?.id ?? null,
          pipelineId: workspace.pipeline?.id ?? null, scoreMutation: false,
        },
        companyId,
        visibility: 'team',
      });
  ...
  return (
    <Card title="Briefing decisório" subtitle="Transforma contexto real da empresa em preparação de reunião e registro auditável" tone="accent" className="dense-card company-decision-brief-card">
      ...
      {savedNode ? <div className="data-banner data-banner-success"><Pill tone="success">salvo</Pill><span>Briefing registrado como nota versionada e vinculado à empresa.</span><Link to="/knowledge-vault">Abrir Vault</Link></div> : null}
      {!loading && workspace ? <>
        ...
        <div className="company-decision-brief-footer">
          <label className="company-decision-confirmation"><input type="checkbox" checked={confirmed} onChange={(event) => setConfirmed(event.target.checked)} />Revisei fatos, inferências, lacunas e fontes antes de salvar.</label>
