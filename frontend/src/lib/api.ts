...
export const api = {
  ...
  getCompanyIntelligence: async (session: SessionData | null, id: string) => toState('Company intelligence', await requestEnvelope<CompanyIntelligenceSummary>(`/company-intelligence/${id}/summary`, session)),
  getCompanyDecisionMemo: async (session: SessionData | null, id: string) => toState('Company decision memo', await requestEnvelope<CompanyDecisionMemo | null>(`/company-decision-memo/${id}`, session)),
  getQualificationBridge: async (session: SessionData | null, id: string) => toState('Qualification bridge', await requestEnvelope<QualificationIntelligenceBridge | null>(`/qualification-intelligence-bridge/${id}`, session)),
  getMvpReadiness: async (session: SessionData | null) => toState('MVP readiness', await requestEnvelope<Record<string, unknown>>('/mvp-readiness', session)),
