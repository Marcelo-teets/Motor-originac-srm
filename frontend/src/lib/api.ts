...
type PaperclipStatusWire = {
  ...
  commands?: PaperclipCommandWire[];
};

const normalizePaperclipCommand = (item: PaperclipCommandWire): AbaCommandRecord => ({
  id: item.id,
  target: item.target,
  action: item.action,
...
const commandPaperclip = async (
  session: SessionData | null,
  target: 'aba' | 'paper_clip' | 'adm',
  action: string,
  context: Record<string, unknown> = {},
) => {
  const envelope = await requestEnvelope<PaperclipCommandWire>('/paperclip', session, {
    method: 'POST',
    body: JSON.stringify({ target, action, context }),
  });
  return normalizePaperclipCommand(envelope.data);
};
...
export const api = {
  ...
  getAbaStatus: async (session: SessionData | null): Promise<DataState<AbaStatus>> => {
    const envelope = await requestEnvelope<PaperclipStatusWire>('/paperclip', session);
    const status = envelope.data.status ?? {};
    return {
      source: envelope.status,
      note: 'Paperclip carregado do control plane real e auditável no Supabase.',
      data: {
        runtime: envelope.data.runtime,
        commandCount: Number(status.command_count ?? 0),
        queued: Number(status.queued ?? 0),
        running: Number(status.running ?? 0),
        completed: Number(status.completed ?? 0),
        failed: Number(status.failed ?? 0),
        lastCommandAt: status.last_command_at ?? null,
        lastCompletedAt: status.last_completed_at ?? null,
        commands: (envelope.data.commands ?? []).map(normalizePaperclipCommand),
      },
    };
  },
  commandAba: (session: SessionData | null, action: string, context: Record<string, unknown> = {}) => commandPaperclip(session, 'aba', action, context),
  commandPaperClip: (session: SessionData | null, action: string, context: Record<string, unknown> = {}) => commandPaperclip(session, 'paper_clip', action, context),
  commandAdm: (session: SessionData | null, action: string, context: Record<string, unknown> = {}) => commandPaperclip(session, 'adm', action, context),
  runAbaAuto: (session: SessionData | null) => commandPaperclip(session, 'paper_clip', 'run_suggested_improvements', { source: 'agents_page' }),
