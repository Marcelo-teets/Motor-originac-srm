...
with watchlist_counts as (
  select company_id, count(distinct watchlist_id) as watchlist_count, max(added_at) as last_watchlist_added_at
  from watchlist_items
  group by company_id
),
activity_stats as (
  select company_id, count(*) filter (where status = 'open') as open_activities, max(created_at) as last_activity_at
  from activities
  group by company_id
),
task_stats as (
  select company_id, count(*) filter (where status in ('todo', 'in_progress', 'blocked')) as open_tasks, max(updated_at) as last_task_at
  from tasks
  group by company_id
)
select
