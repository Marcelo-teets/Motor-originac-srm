...
begin
  ...
  insert into public.database_storage_snapshots (
    ...
    jsonb_build_object(
      ...
      'emergency_mb', 475,
      'free_quota_mb', 500,
      'strategy', 'supabase_hot_google_sheets_cold'
    )
  ...
  return jsonb_build_object(
    ...
    'warning_bytes', v_warning_bytes,
    'critical_bytes', v_critical_bytes,
    'emergency_bytes', v_emergency_bytes,
...
begin
  ...
  if p_trigger_type = 'backfill' and v_state <> 'healthy' then
    raise exception 'storage_budget_blocks_backfill: state %, database_mb %',
      v_state, round(v_database_bytes / 1024.0 / 1024.0, 2)
      using errcode = 'P0001';
  end if;
  ...
  if greatest(coalesce(p_requested_rows, 0), 0) > v_allowed_rows then
    raise exception 'storage_budget_row_limit: state %, requested %, allowed %, database_mb %',
      v_state, p_requested_rows, v_allowed_rows,
      round(v_database_bytes / 1024.0 / 1024.0, 2)
      using errcode = 'P0001';
  end if;
