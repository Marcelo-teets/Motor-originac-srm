-- MVP Closure Gate 2: generated-column-safe candidate resolution + company invalidation.
-- Validated against the live schema in a transaction before promotion.

-- Company changes must invalidate derived artifacts, never synchronously rebuild them.
...
as $$
begin
  perform public.enqueue_company_origination_reprocessing(new.id,'origination_brief_dependency:companies');
  return new;
end;
$$;

create or replace function public.auto_resolve_verified_candidate_entities_v4(p_limit integer default 50)
...
begin
  for q in
    ...
    where c.canonical_rank=1
      ...
      and c.candidate_role in ('operating_company','operating_issuer')
      and coalesce(c.cnpj_valid,false)
  ...
  loop
    ...
    v_now:=now();
    v_cnpj:=regexp_replace(coalesce(q.cnpj,''),'\D','','g');
    ...
    if v_cnpj_count>1 or v_domain_count>1 or
      ...
      set raw_payload=coalesce(d.raw_payload,'{}'::jsonb)||jsonb_build_object(
        'entity_resolution_conflict',jsonb_build_object(
          'version','auto_entity_resolution_v4','status','quarantined','reason','canonical_key_conflict',
          'cnpjCompanyId',v_cnpj_company,'domainCompanyId',v_domain_company,'observedAt',v_now
        )
      ),updated_at=v_now
      where d.id=q.id;
    ...
    if v_company_id is not null then
      ...
      if nullif(v_existing_cnpj,'') is not null and v_existing_cnpj<>v_cnpj then
        ...
        set raw_payload=coalesce(d.raw_payload,'{}'::jsonb)||jsonb_build_object(
          'entity_resolution_conflict',jsonb_build_object(
            'version','auto_entity_resolution_v4','status','quarantined','reason','domain_cnpj_conflict',
            'existingCompanyId',v_company_id,'candidateCnpj',v_cnpj,'existingCnpj',v_existing_cnpj,'observedAt',v_now
          )
        ),updated_at=v_now
        where d.id=q.id;
    ...
    if v_company_id is null then
      -- normalized_name, segment, subsegment, website and geography are generated by PostgreSQL.
      insert into public.companies(
        legal_name,trade_name,cnpj,domain,website_url,sector,sub_sector,
        company_type,origin,metadata,created_at,updated_at
      ) values (
        ...
        v_cnpj,v_domain,q.website,q.segment,q.subsegment,
        coalesce(nullif(q.company_type,''),q.candidate_role,'company'),'automatic_candidate_entity_resolution',
        jsonb_build_object(
          'data_status','real','synthetic_seed',false,
          'identity_verified',true,'identity_review_status','approved','identity_review_method','automatic_deterministic',
          ...
          'decision_eligible',true,'decision_eligibility_reason','verified_entity_ready_for_origination_qualification',
          'origination_analytics_eligible',true,'credit_approval_separate',true,
          'excluded_from_entity_resolution',false,'excluded_from_monitoring',false,
          'excluded_from_qualification',false,'excluded_from_scoring',false,
          'legal_name_provisional',(nullif(btrim(q.legal_name),'') is null),
          'auto_entity_resolution',jsonb_build_object(
            'version','auto_entity_resolution_v4','candidateRole',q.candidate_role,'matchType',v_match_type,
      ...
      set legal_name=coalesce(nullif(c.legal_name,''),nullif(q.legal_name,''),nullif(q.company_name,'')),
          ...
          sector=coalesce(nullif(c.sector,''),q.segment),
          sub_sector=coalesce(nullif(c.sub_sector,''),q.subsegment),
    ...
    values(v_company_id,q.id,'automatic_deterministic_identity_v4',least(1,greatest(0,v_identity_confidence)),
      jsonb_build_object('version','auto_entity_resolution_v4','candidateRole',q.candidate_role,'matchType',v_match_type,'domain',v_domain,'cnpj',v_cnpj),v_now,v_now)
    on conflict (company_id,discovered_candidate_id) do update set
      match_method=excluded.match_method,
      confidence=greatest(public.company_discovery_links.confidence,excluded.confidence),
      metadata=public.company_discovery_links.metadata||excluded.metadata,
      updated_at=v_now;

    update public.discovered_company_candidates d
    set company_id=v_company_id,candidate_status='promoted',promoted_at=coalesce(d.promoted_at,v_now),
        legal_name=coalesce(nullif(d.legal_name,''),nullif(d.company_name,'')),
        raw_payload=coalesce(d.raw_payload,'{}'::jsonb)||jsonb_build_object(
          'legal_name_verified',true,'identity_review_status','approved','identity_review_method','automatic_deterministic',
          ...
          'entityResolution',jsonb_build_object(
            'version','auto_entity_resolution_v4','autoMatched',true,'companyId',v_company_id,
            'method',v_match_type,'confidence',v_identity_confidence,'resolvedAt',v_now
          )
        ),updated_at=v_now
    where d.id=q.id;

    perform public.enqueue_company_origination_reprocessing(v_company_id,'automatic_entity_resolution_v4');
    company_id:=v_company_id; reason:='verified_cnpj_and_first_party_identity'; return next;
  end loop;
end;
$$;

revoke all on function public.auto_resolve_verified_candidate_entities_v4(integer) from public,anon,authenticated;
grant execute on function public.auto_resolve_verified_candidate_entities_v4(integer) to service_role;
revoke execute on function public.auto_resolve_verified_candidate_entities_v3(integer) from service_role;

-- v4 is the only scheduled resolver.
do $$
...
begin
  if exists(select 1 from pg_extension where extname='pg_cron') then
    ...
    perform cron.schedule(
      ...
      '*/15 * * * *',
      $job$select * from public.auto_resolve_verified_candidate_entities_v4(50);$job$
    );
  end if;
end;
$$;
