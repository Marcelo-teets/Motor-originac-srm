-- MVP Closure Gate 2/3 validation fix.
-- Capital-market discovery classifies real operating issuers separately from generic operating companies.
-- Issuers are resolved only with a valid CNPJ plus verified first-party website identity.

create or replace function public.auto_resolve_verified_operating_issuers(p_limit integer default 50)
...
begin
  for q in
    ...
    where c.canonical_rank=1
      ...
      and c.candidate_role='operating_issuer'
  ...
  loop
    ...
    if v_cnpj_count>1 or v_domain_count>1 or
      ...
      set raw_payload=coalesce(d.raw_payload,'{}'::jsonb)||jsonb_build_object(
            'entity_resolution_conflict',jsonb_build_object(
              'version','auto_entity_resolution_v2','status','quarantined',
    ...
    if v_company_id is null then
      insert into public.companies(
        ...
        v_cnpj,v_domain,q.website,q.website,q.segment,q.subsegment,q.segment,q.subsegment,
        q.geography,coalesce(nullif(q.company_type,''),'operating_issuer'),'automatic_candidate_entity_resolution',
        jsonb_build_object(
          ...
          'auto_entity_resolution',jsonb_build_object(
            'version','auto_entity_resolution_v2','candidateRole','operating_issuer','matchType',v_match_type,
      ...
      set legal_name=coalesce(nullif(c.legal_name,''),nullif(q.legal_name,''),nullif(q.company_name,'')),
          ...
          cnpj=coalesce(nullif(c.cnpj,''),v_cnpj),domain=coalesce(nullif(c.domain,''),v_domain),
          website_url=coalesce(nullif(c.website_url,''),q.website),website=coalesce(nullif(c.website,''),q.website),
    ...
    values(v_company_id,q.id,'automatic_deterministic_issuer_identity',least(1,greatest(0,v_identity_confidence)),
      jsonb_build_object('version','auto_entity_resolution_v2','candidateRole','operating_issuer','matchType',v_match_type,'domain',v_domain,'cnpj',v_cnpj),v_now,v_now)
    ...
    set company_id=v_company_id,candidate_status='promoted',promoted_at=coalesce(d.promoted_at,v_now),
        ...
        raw_payload=coalesce(d.raw_payload,'{}'::jsonb)||jsonb_build_object(
          ...
          'entityResolution',jsonb_build_object(
            'version','auto_entity_resolution_v2','autoMatched',true,'companyId',v_company_id,
            'method',v_match_type,'confidence',v_identity_confidence,'resolvedAt',v_now
          )
        ),updated_at=v_now
    where d.id=q.id;

    perform public.enqueue_company_origination_reprocessing(v_company_id,'automatic_issuer_entity_resolution');
    company_id:=v_company_id; reason:='verified_issuer_cnpj_and_first_party_identity'; return next;
  end loop;
end;
$$;

revoke all on function public.auto_resolve_verified_operating_issuers(integer) from public,anon,authenticated;
grant execute on function public.auto_resolve_verified_operating_issuers(integer) to service_role;

-- A verified real entity is eligible for origination analytics even when an older credit-review gate
-- previously excluded it. This is not a credit approval and does not alter reviewed outcomes.
update public.companies c
...
-- Existing generic company resolver + strict issuer resolver share one cadence.
do $$
...
begin
  if exists(select 1 from pg_extension where extname='pg_cron') then
    ...
    perform cron.schedule(
      ...
      '*/15 * * * *',
      $job$select * from public.auto_resolve_verified_candidate_entities(25); select * from public.auto_resolve_verified_operating_issuers(50);$job$
    );
  end if;
end;
$$;
...
select c.id,'queued',jsonb_build_array('operating_issuer_resolution_fix'),now(),now(),now()
from public.companies c where public.is_company_decision_eligible(c.id)
