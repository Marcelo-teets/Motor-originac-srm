...
begin
  v_dataset_code := coalesce(
    new.metadata ->> 'datasetCode',
    new.evidence_payload ->> 'datasetCode'
  );

  if v_dataset_code <> 'rfb_qsa' then
    return new;
  end if;

  -- A baseline QSA snapshot is monitoring evidence, not an origination signal.
  if new.signal_type = 'ownership_structure_signal' then
    return null;
  end if;

  if new.signal_type = 'ownership_change' then
    select coalesce((company.metadata ->> 'decision_eligible')::boolean, false)
      into v_decision_eligible
    ...
    v_source_authority := coalesce(
      new.evidence_payload #>> '{normalized,sourceAuthority}',
      new.metadata ->> 'sourceAuthority',
      'official_primary'
    );
...
set status = 'partial',
    ...
    metadata = coalesce(metadata, '{}'::jsonb) || jsonb_build_object(
      ...
      'officialBulkHealth', 'degraded',
      'officialBulkLastHttpStatus', 404,
      'fallbackSourceCode', 'src_brasilapi_cnpj',
      'fallbackProbePassed', true,
      'fallbackProbeRecords', 8,
      'companyMasterRealCnpjReady', true,
