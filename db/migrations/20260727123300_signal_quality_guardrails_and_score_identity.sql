...
begin
  if tg_op = 'UPDATE' then
    if new.observed_vs_inferred is distinct from old.observed_vs_inferred then
      normalized_mode := pg_catalog.lower(new.observed_vs_inferred);
    elsif new.metadata ->> 'observedVsInferred'
          is distinct from old.metadata ->> 'observedVsInferred' then
      normalized_mode := pg_catalog.lower(new.metadata ->> 'observedVsInferred');
    else
