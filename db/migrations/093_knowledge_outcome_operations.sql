...
begin
  ...
  with pending_outcomes as (
    ...
    select
      ...
      t.owner_name,
      t.metadata,
      (t.metadata->>'knowledgeActivityId')::uuid as knowledge_activity_id
    from public.tasks t
    ...
    where p.status = 'active'
      ...
      and (
        nullif(btrim(coalesce(p.next_action, '')), '') is null
        or p.next_action_due_at < now()
      )
    ...
    from public.activities a
    join public.companies c on c.id = a.company_id
    left join public.pipeline p
      on p.id = a.pipeline_id
      or (a.pipeline_id is null and p.company_id = a.company_id)
  ...
  select jsonb_build_object(
    ...
    'adoptionCandidates', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'companyId', q.company_id,
        'companyName', q.company_name,
        'pipelineId', q.effective_pipeline_id,
        'activityType', q.activity_type,
        'title', q.title,
        'description', q.description,
        'ownerName', q.owner_name,
        'occurredAt', q.occurred_at,
        'ageDays', q.age_days,
        'canAdopt', q.effective_pipeline_id is not null
      ) order by q.occurred_at desc)
      from (
        select * from adoption_candidates
        where effective_pipeline_id is not null
...
begin
  ...
  select * into existing_reference
  ...
  if found then
    return jsonb_build_object(
      'status', 'already_instrumented',
      'activityId', activity_row.id,
      'companyId', activity_row.company_id,
      'nodeId', existing_reference.node_id,
  ...
  from public.pipeline p
  where p.id = activity_row.pipeline_id
     or (activity_row.pipeline_id is null and p.company_id = activity_row.company_id)
  ...
  if p_node_id is not null then
    ...
    where id = p_node_id
      and company_id = activity_row.company_id
      and status = 'active';
    ...
    generated_content := format(
      ...
      coalesce(activity_row.owner_name, 'não informado'),
      coalesce(nullif(activity_row.description, ''), activity_row.title)
    );
    ...
    node_payload := public.knowledge_save_node(
      ...
      jsonb_build_object(
        'source', 'legacy_activity_adoption',
        'activityId', activity_row.id,
        'contextMode', 'reconstructed_at_adoption',
        'originalOccurredAt', activity_row.occurred_at
      ),
    ...
    select * into node_row
    from public.knowledge_nodes
    where id = (node_payload->'node'->>'id')::uuid;
  end if;
  ...
  context_snapshot := context_snapshot || jsonb_build_object(
    ...
    'actionRequest', jsonb_build_object(
      'fromStage', null,
      'requestedStage', null,
      'effectiveStage', pipeline_row.stage,
      'requestedNextAction', null,
      'effectiveNextAction', pipeline_row.next_action,
      'dueAt', null
    )
  );
  ...
  set
    ...
    metadata = metadata || jsonb_build_object(
      'status', coalesce(metadata->>'status', 'open'),
      'knowledgeNodeId', node_row.id,
      'outcomeInstrumentationOrigin', 'knowledge_vault_v7',
      'outcomeContext', context_snapshot,
      'instrumentedAt', now(),
      'instrumentedBy', current_user_id,
  ...
  insert into public.tasks (
    ...
    activity_row.company_id,
    pipeline_row.id,
    'Registrar resultado — ' || activity_row.title,
    ...
    jsonb_build_object(
      ...
      'knowledgeNodeId', node_row.id,
      'knowledgeActivityId', activity_row.id,
      'adoptedLegacyActivity', true,
  ...
  insert into public.knowledge_references (
    ...
    'Registrar resultado — ' || activity_row.title,
    jsonb_build_object(
      'title', 'Registrar resultado — ' || activity_row.title,
      'status', 'todo',
      'dueAt', now(),
      'activityId', activity_row.id,
      'contextMode', 'reconstructed_at_adoption',
  ...
  return jsonb_build_object(
    ...
    'activityId', activity_row.id,
    'companyId', activity_row.company_id,
    'pipelineId', pipeline_row.id,
    'nodeId', node_row.id,
    'nodeTitle', node_row.title,
    'taskId', tracking_task_id,
    'contextMode', 'reconstructed_at_adoption'
  );
end;
$$;
