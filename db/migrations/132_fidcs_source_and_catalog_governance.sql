-- Adds FIDCS.com.br as a bounded secondary validation source and separates
-- operational incidents from planned or superseded connectors.
-- CVM remains the canonical regulatory source.
...
values (
  ...
  jsonb_build_object(
    'code', 'src_fidcs_com_br',
    'provider', 'fidcs.com.br',
    'official', false,
    'freePublicAccess', true,
    'tier', 'tier_5_supplemental_enrichment',
    'sourceAuthority', 'derived_secondary',
    'canonicalUpstream', 'CVM',
    'entityKey', 'fund_cnpj',
    'captureMode', 'bounded_public_ssr_validation',
    'publicRoutes', jsonb_build_array('/fundos', '/fundo/{cnpj}'),
    'optionalPremiumSessionEnv', 'FIDCS_SESSION_COOKIE',
    ...
    'implementationPhase', 'public_validation_runtime',
    'runtime', 'fidcs-com-br-v1',
    'scheduler', 'manual_or_external_cron',
    'addedAt', '2026-07-28T00:00:00-03:00'
  ),
  'public_ssr_optional_premium',
  'none_public_optional_session_for_premium',
...
set status = 'retired',
    ...
    metadata = source.metadata || jsonb_build_object(
      ...
      'supersededBy', duplicates.superseded_by,
      'supersededReason', duplicates.reason,
      'supersededAt', '2026-07-28T00:00:00-03:00'
    ),
    updated_at = now()
from duplicates
...
set health = case when coalesce((metadata ->> 'fallbackProbePassed')::boolean, false) then 'healthy' else health end,
    metadata = metadata || jsonb_build_object(
      'effectiveRuntime', case when coalesce((metadata ->> 'fallbackProbePassed')::boolean, false)
        then 'fallback_operational' else 'official_bulk_degraded' end,
...
begin
  ...
  if v_record_key is null or length(v_cnpj) <> 14 then
    raise exception 'FIDCS snapshot requires publicRecordKey and a valid CNPJ';
  end if;

  select id into v_source_id
  from public.source_catalog
  where metadata ->> 'code' = 'src_fidcs_com_br'
  limit 1;

  if v_source_id is null then
    raise exception 'FIDCS.com.br source is missing from source_catalog';
  end if;

  select id into v_output_id
  from public.monitoring_outputs
  where source_id = v_source_id
    and payload ->> 'publicRecordKey' = v_record_key
  ...
  if v_output_id is null then
    ...
    insert into public.monitoring_outputs (
      ...
      null,
      concat_ws(' · ',
        coalesce(p_snapshot ->> 'fundName', p_snapshot ->> 'legalName'),
        p_snapshot ->> 'cnpjFormatted',
        p_snapshot ->> 'status',
      ...
      p_snapshot || jsonb_build_object(
        'datasetCode', 'fidcs_com_br_public_validation',
        'sourceCode', 'src_fidcs_com_br',
        'publicRecordKey', v_record_key,
        'canonicalUpstream', 'CVM',
      ...
      jsonb_build_object(
        ...
        'manager', p_snapshot ->> 'manager',
        'administrator', p_snapshot ->> 'administrator',
        'netAssetValueBrl', p_snapshot -> 'netAssetValueBrl',
        'shareholdersCount', p_snapshot -> 'shareholdersCount',
        'defaultRatePercent', p_snapshot -> 'defaultRatePercent',
    ...
    set title = concat('FIDCS.com.br · ', coalesce(p_snapshot ->> 'fundName', p_snapshot ->> 'legalName', v_cnpj)),
        ...
        payload = p_snapshot || jsonb_build_object(
          'datasetCode', 'fidcs_com_br_public_validation',
          'sourceCode', 'src_fidcs_com_br',
          'publicRecordKey', v_record_key,
          'canonicalUpstream', 'CVM',
        ...
        normalized_payload = p_snapshot,
        confidence_score = 0.75,
        connector_status = 'real',
...
as $$
  with source as (
    select id, name, status, health, metadata, updated_at
    from public.source_catalog
    where metadata ->> 'code' = 'src_fidcs_com_br'
    limit 1
  ), latest_run as (
    select run.*
    from public.source_connector_runs run
    ...
    join source on source.id = output.source_id
    where output.payload ->> 'sourceCode' = 'src_fidcs_com_br'
  )
  select jsonb_build_object(
    'status', coalesce(source.status, 'partial'),
    'health', coalesce(source.health, 'degraded'),
    'sourceId', source.id,
    'sourceName', source.name,
    'canonicalUpstream', 'CVM',
    'sourceAuthority', 'derived_secondary',
    'automaticScoreImpact', false,
    'outputCount', coalesce(outputs.output_count, 0),
    'lastOutputAt', outputs.last_output_at,
    'lastRun', case when latest_run.id is null then null else jsonb_build_object(
      ...
      'itemsCollected', latest_run.items_collected,
      'outputsWritten', latest_run.outputs_written,
      'errorMessage', latest_run.error_message,
...
comment on function public.persist_fidcs_validation(jsonb) is
  'Persists one lightweight FIDCS.com.br public validation snapshot without overriding canonical CVM records.';
comment on function public.fidcs_runtime_status() is
  'Returns the operational status of the bounded FIDCS.com.br secondary validation connector.';
