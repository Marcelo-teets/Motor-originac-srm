...
begin
  ...
  if not public.is_company_entity_eligible(new.company_id) then return new; end if;

  select * into cfg
  from public.trigger_catalog c
  where c.active and new.signal_type=any(c.signal_types)
  order by c.min_strength desc
  limit 1;
  if cfg.code is null then return new; end if;

  v_strength:=least(100,greatest(0,coalesce(new.signal_strength,new.strength,0)));
  v_confidence:=coalesce(new.confidence_score,case when coalesce(new.confidence,0)>1 then new.confidence/100.0 else new.confidence end,0);
  v_confidence:=least(1,greatest(0,v_confidence));
  ...
  v_material:=cfg.material_default
    and v_strength>=cfg.min_strength
    and v_confidence>=cfg.min_confidence
    and v_observed>=now()-make_interval(days=>cfg.staleness_days);

  -- One economic trigger family per company/day. Repeated evidence can increase strength,
  -- but never multiplies the score simply because the same fact was captured many times.
  ...
  insert into public.trigger_events(
    ...
    new.company_id,new.id,cfg.trigger_type,v_final_strength,
    coalesce(nullif(new.signal_label,''),cfg.trigger_type),
    coalesce(nullif(new.evidence_text,''),'Trigger derivado de sinal persistido com lineage.'),
    v_observed,
    jsonb_build_object('engineVersion','material_trigger_v1','signalType',new.signal_type,'rawStrength',v_strength,'explicitMultiplier',v_explicit_multiplier),
    now(),new.source_id,v_confidence,
    jsonb_build_object(
      'signalId',new.id,'monitoringOutputId',new.monitoring_output_id,'sourceId',new.source_id,
      'evidenceUrl',new.evidence_url,'evidenceText',new.evidence_text,
...
begin
  ...
  select * into q from public.qualification_snapshots
  where company_id=p_company_id order by created_at desc,id desc limit 1;
  ...
  if q.id is null then
    ...
    insert into public.qualification_snapshots(
      company_id,snapshot_version,has_credit_product,has_receivables,has_fidc,uses_structured_debt,
      funding_gap,fit_fidc,fit_dcm,timing,structural_need_score,timing_score,executability_score,total_score,
      rationale,next_action,evidence,created_by,qualification_score_total,urgency_score,
      predicted_funding_need_score,source_confidence_score,trigger_strength_score,confidence_score,
      qualification_score_structural,qualification_score_execution,qualification_score_timing,evidence_payload,created_at
    )
    select c.id,'p2_trigger_v1',c.credit_product,c.has_receivables,c.has_fidc,c.has_structured_debt,
      c.funding_gap,c.fit_fidc,c.fit_dcm,'material_trigger',v_qscore,v_timing,v_execution,v_qscore,
      'Qualification baseline criada por trigger material; campos sem evidência permanecem conservadores.',
      'Validar funding, recebíveis, ticket, prazo e responsável financeiro.',
      jsonb_build_object('materialTrigger',to_jsonb(t)),'material_trigger_engine',v_qscore,v_timing,
      greatest(v_qscore,t.trigger_strength),v_conf,t.trigger_strength,v_conf,
    ...
    insert into public.qualification_snapshots(
      company_id,snapshot_version,has_credit_product,credit_is_core,has_receivables,receivables_structurable,
      has_fidc,uses_structured_debt,capital_structure_fit,funding_gap,fit_fidc,fit_dcm,timing,
      structural_need_score,timing_score,executability_score,total_score,rationale,next_action,evidence,created_by,
      qualification_score_total,urgency_score,suggested_structure_type,predicted_funding_need_score,
      source_confidence_score,trigger_strength_score,pattern_summary,credit_product_type,credit_is_core_product,
      receivables_type,receivables_recurrence_level,receivables_predictability_level,has_securitization_structure,
      has_existing_debt_structure,funding_structure_type,capital_structure_quality,capital_structure_rationale,
      funding_gap_level,capital_dependency_level,growth_vs_funding_mismatch,fit_other_structure,
      governance_maturity_level,risk_model_maturity_level,underwriting_maturity_level,operational_maturity_level,
      unit_economics_quality,spread_vs_funding_quality,concentration_risk_level,delinquency_signal_level,
      timing_intensity_level,execution_readiness_level,qualification_score_structural,qualification_score_capital,
      qualification_score_receivables,qualification_score_execution,qualification_score_timing,confidence_score,
      rationale_summary,evidence_payload,created_at
    ) values (
      q.company_id,'p2_trigger_v1',q.has_credit_product,q.credit_is_core,q.has_receivables,q.receivables_structurable,
      q.has_fidc,q.uses_structured_debt,q.capital_structure_fit,q.funding_gap,q.fit_fidc,q.fit_dcm,'material_trigger',
      q.structural_need_score,v_timing,v_execution,v_qscore,
      concat_ws(' ',q.rationale,'Trigger material '||t.trigger_type||' incorporado com dedupe e staleness.'),
      q.next_action,coalesce(q.evidence,'{}'::jsonb)||jsonb_build_object('materialTrigger',to_jsonb(t)),'material_trigger_engine',
      v_qscore,greatest(coalesce(q.urgency_score,0),v_timing),q.suggested_structure_type,
      greatest(coalesce(q.predicted_funding_need_score,0),case when t.catalog_code='funding_gap' then t.trigger_strength else 0 end),
      v_conf,t.trigger_strength,q.pattern_summary,q.credit_product_type,q.credit_is_core_product,
      q.receivables_type,q.receivables_recurrence_level,q.receivables_predictability_level,q.has_securitization_structure,
      q.has_existing_debt_structure,q.funding_structure_type,q.capital_structure_quality,q.capital_structure_rationale,
      q.funding_gap_level,q.capital_dependency_level,q.growth_vs_funding_mismatch,q.fit_other_structure,
      q.governance_maturity_level,q.risk_model_maturity_level,q.underwriting_maturity_level,q.operational_maturity_level,
      q.unit_economics_quality,q.spread_vs_funding_quality,q.concentration_risk_level,q.delinquency_signal_level,
      case when v_timing>=75 then 'high' when v_timing>=55 then 'medium' else q.timing_intensity_level end,
      q.execution_readiness_level,q.qualification_score_structural,q.qualification_score_capital,
      q.qualification_score_receivables,v_execution,v_timing,greatest(coalesce(q.confidence_score,0),v_conf),
      concat_ws(' ',q.rationale_summary,'Atualizado por trigger material.'),
      coalesce(q.evidence_payload,'{}'::jsonb)||jsonb_build_object('materialTrigger',to_jsonb(t)),now()
    );
  end if;
  ...
  v_bucket:=case when v_lead>=85 then 'immediate_priority' when v_lead>=70 then 'high_priority' when v_lead>=55 then 'monitor_closely' when v_lead>=40 then 'watchlist' else 'low_priority' end;

  insert into public.lead_score_snapshots(
    company_id,lead_score,priority_tier,commercial_angle,suggested_structure,next_action,rationale,
    bucket,source_confidence,trigger_strength,pattern_score,created_at
  ) values (
    p_company_id,v_lead,v_bucket,
    'Prioridade recalculada por trigger material com evidência persistida e staleness controlado.',
    q.suggested_structure_type,q.next_action,
    'Lead score p2_trigger_v1: qualification 70%, trigger 20%, source confidence 10%.',
...
as $$
begin
  if new.material then perform public.recalculate_decision_layers_from_trigger(new.company_id); end if;
  return new;
end;
$$;
...
-- Rebuild ranking with explicit staleness decay. Old triggers can never dominate a fresh structural score.
...
begin
  with latest_qualification as (
    select distinct on (qualification.company_id)
      qualification.company_id,qualification.qualification_score_total,qualification.trigger_strength_score,
      qualification.source_confidence_score,qualification.evidence_payload
    ...
    select distinct on (lead.company_id) lead.company_id,lead.lead_score
    from public.lead_score_snapshots lead
    where lead.lead_score is not null and public.is_company_decision_eligible(lead.company_id)
    order by lead.company_id,lead.created_at desc,lead.id desc
  ), latest_pattern as (
    select distinct on (pattern.company_id,pattern.pattern_id) pattern.company_id,pattern.pattern_id,pattern.ranking_impact
    ...
    select distinct on (t.company_id) t.company_id,t.trigger_strength,t.occurred_at,
      case when t.occurred_at>=now()-interval '30 days' then 1.0
           when t.occurred_at>=now()-interval '90 days' then 0.75
           when t.occurred_at>=now()-interval '180 days' then 0.50 else 0 end as freshness
    from public.trigger_events t
    where t.material and public.is_company_decision_eligible(t.company_id)
    order by t.company_id,t.trigger_strength desc,t.occurred_at desc,t.id desc
  ), scored as (
    select qualification.company_id,qualification.qualification_score_total::integer qualification_score,
      lead.lead_score::integer lead_score,
      greatest(0,least(100,round(
        qualification.qualification_score_total*0.40+
        lead.lead_score*0.35+
        coalesce(trigger_state.trigger_strength*trigger_state.freshness,0)*0.10+
      ...
      coalesce(trigger_state.freshness,0) trigger_freshness
    ...
    left join trigger_state on trigger_state.company_id=qualification.company_id
  ), guarded as (
    select company_id,qualification_score,lead_score,
      case when blocking_risk_count>0 then least(raw_ranking_score,54)
           when risk_level='high' then least(raw_ranking_score,69) else raw_ranking_score end::integer ranking_score,
      risk_level,trigger_freshness from scored
  ), positioned as (
    select company_id,row_number() over(order by ranking_score desc,company_id asc)::integer position,
      qualification_score,lead_score,ranking_score,risk_level,trigger_freshness from guarded
  ), latest_snapshot as (select max(created_at) created_at from public.ranking_v2),
  latest_rows as (
    select r.company_id,r.position,r.qualification_score,r.lead_score,r.ranking_score
    from public.ranking_v2 r join latest_snapshot s on r.created_at=s.created_at
  ), changes as (
...
-- Backfill only currently relevant signal history. The daily dedupe key prevents score inflation.
insert into public.trigger_events(
  company_id,signal_id,trigger_type,trigger_strength,title,description,occurred_at,metadata,created_at,
  source_id,confidence_score,evidence_payload,dedupe_key,material,stale_after,catalog_code
)
select distinct on (s.company_id,c.code,(coalesce(s.observed_at,s.created_at) at time zone 'UTC')::date)
  ...
  jsonb_build_object('engineVersion','material_trigger_v1','backfill',true,'signalType',s.signal_type),now(),
  s.source_id,
  least(1,greatest(0,coalesce(s.confidence_score,case when coalesce(s.confidence,0)>1 then s.confidence/100.0 else s.confidence end,0))),
...
where public.is_company_entity_eligible(s.company_id)
  and coalesce(s.observed_at,s.created_at)>=now()-interval '180 days'
order by s.company_id,c.code,(coalesce(s.observed_at,s.created_at) at time zone 'UTC')::date,
  coalesce(s.signal_strength,s.strength,0)*least(1,greatest(0,coalesce(s.confidence_score,case when coalesce(s.confidence,0)>1 then s.confidence/100.0 else s.confidence end,0))) desc,
  s.created_at desc
on conflict (dedupe_key) where dedupe_key is not null do update set
  trigger_strength=greatest(public.trigger_events.trigger_strength,excluded.trigger_strength),
  confidence_score=greatest(coalesce(public.trigger_events.confidence_score,0),coalesce(excluded.confidence_score,0)),
...
-- One recalculation per currently ranked-eligible company after the backfill.
do $$
declare r record;
begin
