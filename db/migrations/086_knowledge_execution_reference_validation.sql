-- Knowledge Vault V5 hardening: extend reference validation to the existing CRM entities.
-- Preserves all previous checks and requires activity/task ownership to match the note company.
  ...
  case new.reference_type
    ...
    when 'activity' then
      if not exists (
        ...
        raise exception 'Activity reference is invalid';
      end if;
    when 'task' then
      if not exists (
        ...
        raise exception 'Task reference is invalid';
      end if;
    else
...
comment on function public.validate_knowledge_reference()
is 'Validates that every knowledge reference exists and belongs to the same company as its active knowledge node.';
