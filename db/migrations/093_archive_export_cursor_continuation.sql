create or replace function public.continue_historical_excel_export_cursor(
  p_run_id uuid,
  p_cursor text,
...
begin
  ...
  if nullif(p_cursor, '') is null or p_part_number < 2 then
    raise exception 'invalid_archive_cursor_continuation';
  end if;
...
revoke all on function public.continue_historical_excel_export_cursor(uuid, text, integer)
  from public, anon, authenticated;
grant execute on function public.continue_historical_excel_export_cursor(uuid, text, integer)
  to service_role;
