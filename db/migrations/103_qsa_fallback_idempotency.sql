-- Follow-up to 102: make monthly QSA monitoring outputs idempotent and report existing evidence.
...
begin
  ...
  from jsonb_array_elements(p_records) item(record)
  where not exists (
    select 1 from public.monitoring_outputs existing
  ...
  where existing.company_id = p_company_id
    ...
    and existing.payload ->> 'publicRecordKey' in (
      select item.record ->> 'recordKey' from jsonb_array_elements(p_records) item(record)
    );
  ...
  return jsonb_build_object(
    ...
    'outputsAvailable', v_outputs_available, 'signalsWritten', 0, 'observedAt', p_observed_at
  );
end;
$$;
