...
grant update (role, status) on table public.user_profiles to authenticated;
...
comment on function public.set_user_access(uuid, text, text) is 'GOD-MODE-only access administration executed with caller RLS and guarded by the profile trigger.';
