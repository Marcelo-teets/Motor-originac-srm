-- MVP Closure Gate 2: unified deterministic resolver.
-- v3 fixes generated-column compatibility and keeps CNPJ as the mandatory canonical key
-- for automatic promotion. Ambiguous identities remain quarantined inside the treatment engine.

create or replace function public.auto_resolve_verified_candidate_entities_v3(p_limit integer default 50)
...
begin
  ...
  loop
    ...
    if v_cnpj_count>1 or v_domain_count>1 or
      ...
      set raw_payload=coalesce(d.raw_payload,'{}'::jsonb)||jsonb_build_object(
            'entity_resolution_conflict',jsonb_build_object(
              'version','auto_entity_resolution_v3','status','quarantined',
    ...
    if v_company_id is null then
      -- normalized_name is generated by PostgreSQL and must never be explicitly inserted.
      insert into public.companies(
        legal_name,trade_name,cnpj,domain,website_url,website,
        ...
        q.geography,coalesce(nullif(q.company_type,''),q.candidate_role,'company'),'automatic_candidate_entity_resolution',
        jsonb_build_object(
          ...
          'auto_entity_resolution',jsonb_build_object(
            'version','auto_entity_resolution_v3','candidateRole',q.candidate_role,'matchType',v_match_type,
    ...
    values(v_company_id,q.id,'automatic_deterministic_identity_v3',least(1,greatest(0,v_identity_confidence)),
      jsonb_build_object('version','auto_entity_resolution_v3','candidateRole',q.candidate_role,'matchType',v_match_type,'domain',v_domain,'cnpj',v_cnpj),v_now,v_now)
    ...
    set company_id=v_company_id,
        ...
        raw_payload=coalesce(d.raw_payload,'{}'::jsonb)||jsonb_build_object(
          ...
          'entityResolution',jsonb_build_object(
            'version','auto_entity_resolution_v3','autoMatched',true,'companyId',v_company_id,
    ...
    perform public.enqueue_company_origination_reprocessing(v_company_id,'automatic_entity_resolution_v3');
    company_id:=v_company_id; reason:='verified_cnpj_and_first_party_identity'; return next;
  end loop;
end;
$$;

revoke all on function public.auto_resolve_verified_candidate_entities_v3(integer) from public,anon,authenticated;
grant execute on function public.auto_resolve_verified_candidate_entities_v3(integer) to service_role;

-- Remove runtime execution rights from superseded resolvers. Their definitions remain for migration history only.
revoke execute on function public.auto_resolve_verified_candidate_entities(integer) from service_role;
revoke execute on function public.auto_resolve_verified_operating_issuers(integer) from service_role;

-- Only v3 is scheduled going forward.
do $$
...
begin
  if exists(select 1 from pg_extension where extname='pg_cron') then
    ...
    perform cron.schedule(
      ...
      '*/15 * * * *',
      $job$select * from public.auto_resolve_verified_candidate_entities_v3(50);$job$
    );
  end if;
end;
$$;
