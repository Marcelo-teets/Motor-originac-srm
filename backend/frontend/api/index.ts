...
type CaptureRuntimeRecordInput = {
...
async function insertRuntimeRecord(input: CaptureRuntimeRecordInput) {
  const baseUrl = process.env.SUPABASE_URL;
  const key = supabaseKey();
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(`Backend entrypoint runtime record failed: ${response.status} ${await response.text()}`);
    }
  } catch (error) {
    console.warn('Backend entrypoint runtime record failed:', error instanceof Error ? error.message : error);
  }
}
...
async function runCaptureRuntime(req: IncomingMessage, res: ServerResponse, triggerType: 'cron' | 'manual') {
  ...
  try {
    ...
    await insertRuntimeRecord({
      ...
      metadata: {
        auditVersion: 'backend_entrypoint_v1',
        reason: triggerType === 'cron' ? 'vercel_cron' : 'manual_serverless_runtime',
