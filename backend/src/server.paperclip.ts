...
app.get('/health', (_req, res) => res.json(ok(platformMode, { service: 'backend-paperclip', mode: platformMode, uptime: process.uptime() })));
...
app.get('/platform/status', wrap(async (_req, res) => res.json(ok(platformMode, {
  ...
  paperclipControlPlane: 'partial',
  persistence: platformMode,
}))));

app.listen(env.port, () => {
  console.log(`Motor backend + Paperclip control plane listening on http://localhost:${env.port}`);
});
