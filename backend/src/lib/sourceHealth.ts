...
const sourceDecision = (source: SourceCatalogEntry, outputs: MonitoringOutput[], runs: SourceConnectorRun[], official: boolean) => {
  if (source.health === 'down') return { decision: 'source_down', nextAction: 'Review adapter, URL or credential before using this source in score.' };
  if (source.status === 'planned') return { decision: 'planned', nextAction: 'Prioritize implementation only if it improves real origination coverage.' };

  const failedRuns = runs.filter((run) => run.status === 'failed').length;
  if (runs.length && failedRuns >= Math.ceil(runs.length / 2)) {
    return { decision: 'recurrent_failures', nextAction: 'Inspect source_connector_runs errors and connector payloads.' };
  }
  if (!outputs.length) {
    return { decision: 'no_recent_evidence', nextAction: official ? 'Run official capture and validate Supabase persistence.' : 'Run capture and check company/source matching.' };
  }
  const partialCount = outputs.filter((item) => item.connectorStatus !== 'real').length;
  if (partialCount > outputs.length / 2) return { decision: 'partial_outputs', nextAction: 'Investigate partial connectors and error payloads.' };
  return { decision: 'operational', nextAction: 'Use evidence in qualification, patterns and ranking.' };
};
