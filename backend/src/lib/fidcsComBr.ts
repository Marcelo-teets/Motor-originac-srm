...
export type FidcsFundSnapshot = {
  ...
  netAssetValueBrl: number | null;
  shareholdersCount: number | null;
  defaultRatePercent: number | null;
  pddPercent: number | null;
  sourceUrl: string;
  observedAt: string;
  contentHash: string;
  providerEdgeWarning: boolean;
...
const parseMoney = (value: string | null, scale: string | null) => {
  ...
  const multiplier = /bi/i.test(scale ?? '') ? 1_000_000_000
    : /mi/i.test(scale ?? '') ? 1_000_000
      : /mil/i.test(scale ?? '') ? 1_000 : 1;
...
export const parseFidcsFundHtml = (
  ...
  const fundName = capture(text, /O que é o\s+(.+?)\?/i)
    ?? heading?.replace(/^Fundo\s+/i, '').replace(/\s+-\s+\d{2}\.\d{3}\.\d{3}\/\d{4}-\d{2}.*$/, '').trim()
    ?? null;
  ...
  const administrator = capture(text, /e administrado por\s+(.+?)(?:\.\s|\.\.\s| Possui| O administrador)/i)
