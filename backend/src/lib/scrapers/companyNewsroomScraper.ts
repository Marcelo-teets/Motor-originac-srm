...
const paths = ['/blog', '/newsroom', '/noticias', '/press', '/imprensa'];
...
const fetchPage = async (url: string): Promise<ScrapedPage | null> => {
  try {
    ...
    return { url, pageType: 'newsroom', title, headings, excerpt: rawText.slice(0, 280), rawText, status: 'real' };
  } catch {
    return null;
  }
};

export const scrapeCompanyNewsroom = async (params: { companyId: string; companyName: string; website: string; }): Promise<B2BSignalPackResult> => {
  ...
  return {
    ...
    consolidatedText,
    signals,
    metadata: { baseUrl, pagesVisited: pages.length, candidatePaths: urls.length, visitedPageTypes: ['newsroom'] },
  };
};
