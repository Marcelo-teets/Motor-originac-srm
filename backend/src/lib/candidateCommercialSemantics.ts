...
const GENERIC_EDITORIAL_NAMES = new Set([
  'entrevista', 'noticia', 'noticias', 'opiniao', 'podcast', 'especial', 'exclusivo',
  'conteudo patrocinado', 'publicidade', 'publieditorial', 'mercado', 'setor', 'credito',
  'fidc', 'fidcs', 'fintech', 'fintechs', 'startup', 'startups', 'antecipacao de recebiveis',
]);
...
const DIRECT_FUNDING_ACTION = /\b(capta|captou|levanta|levantou|busca|obtem|obteve|recebe|recebeu|garante|garantiu|contrata|contratou|assegura|assegurou)\b/i;
const FUNDING_CLOSE_ACTION = /\b(fecha|fechou)\b/i;
const NON_FUNDING_AGREEMENT_CONTEXT = /\b(acordo|parceria|convenio|convênio|memorando|mou|integracao|integração|contrato\s+comercial)\b/i;
...
const parseLocalizedNumber = (value: string) => {
  ...
  if (compact.includes(',') && compact.includes('.')) return Number(compact.replace(/\./g, '').replace(',', '.'));
...
export const extractFundingAmount = (text: string): CandidateCommercialSemantics['fundingAmount'] => {
  ...
  const scaleRaw = String(match[3] ?? '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
  const scale = /bilh|^bi$/.test(scaleRaw) ? 1_000_000_000
    : /milh|^mi$/.test(scaleRaw) ? 1_000_000
      : scaleRaw === 'mil' ? 1_000 : 1;
  return {
...
export const classifyCandidateCommercialSemantics = (
  ...
  if (!normalizedName || GENERIC_EDITORIAL_NAMES.has(normalizedName)) {
    return {
      version: 3, mediaCandidate: true, candidateRole: 'non_entity', commercialQueue: false,
      signalClass: 'editorial_noise', reason: 'generic_editorial_subject_not_company', confidence: 0.99,
      title, discoveryLane, fundingInstrument: instrument, fundingAmount: amount,
      explicitFundingNeed: false, explicitCreditExpansion: false, automaticDecisionEligible: false,
    };
  }
  ...
  if (intermediary) {
    return {
      version: 3, mediaCandidate: true, candidateRole: 'financial_intermediary', commercialQueue: false,
      signalClass: 'market_intermediary_activity', reason: 'explicit_structuring_for_third_party', confidence: 0.94,
      title, discoveryLane, fundingInstrument: instrument, fundingAmount: amount,
      explicitFundingNeed: false, explicitCreditExpansion: false, automaticDecisionEligible: false,
    };
  }
  ...
  if (fundingPlan) {
    return {
      version: 3, mediaCandidate: true, candidateRole: 'operating_company', commercialQueue: true,
      signalClass: 'funding_plan_trigger', reason: 'explicit_credit_funding_plan_without_completed_raise', confidence: 0.86,
      title, discoveryLane, fundingInstrument: instrument, fundingAmount: amount,
      explicitFundingNeed: true, explicitCreditExpansion: true, automaticDecisionEligible: false,
    };
  }

  const explicitFundingAction = DIRECT_FUNDING_ACTION.test(normalizedTitle)
    || (FUNDING_CLOSE_ACTION.test(normalizedTitle)
      && Boolean(instrument)
      && !NON_FUNDING_AGREEMENT_CONTEXT.test(normalizedTitle));
  const directFunding = explicitFundingAction
    && Boolean(instrument || (amount && CREDIT_CONTEXT.test(normalizedTitle)));
  if (directFunding) {
    return {
      version: 3, mediaCandidate: true, candidateRole: 'operating_company', commercialQueue: true,
      signalClass: 'direct_funding_trigger', reason: instrument
        ? 'explicit_funding_action_with_credit_instrument'
        : 'explicit_funding_action_amount_and_credit_context',
      confidence: instrument ? 0.97 : 0.92,
      title, discoveryLane, fundingInstrument: instrument, fundingAmount: amount,
      explicitFundingNeed: true, explicitCreditExpansion: CREDIT_CONTEXT.test(normalizedTitle), automaticDecisionEligible: false,
    };
  }

  const creditExpansion = CREDIT_CONTEXT.test(normalizedTitle) && CREDIT_EXPANSION_ACTION.test(normalizedTitle);
  if (creditExpansion) {
    return {
      version: 3, mediaCandidate: true, candidateRole: 'operating_company', commercialQueue: false,
      signalClass: 'credit_expansion_trigger', reason: 'explicit_credit_or_receivables_expansion_without_funding_need', confidence: 0.82,
      title, discoveryLane, fundingInstrument: instrument, fundingAmount: amount,
      explicitFundingNeed: false, explicitCreditExpansion: true, automaticDecisionEligible: false,
    };
  }

  return {
    version: 3, mediaCandidate: true, candidateRole: 'needs_classification', commercialQueue: false,
    signalClass: 'relevant_unclassified', reason: 'relevant_media_signal_without_explicit_direct_funding_semantics', confidence: 0.70,
    title, discoveryLane, fundingInstrument: instrument, fundingAmount: amount,
    explicitFundingNeed: false, explicitCreditExpansion: false, automaticDecisionEligible: false,
  };
};
