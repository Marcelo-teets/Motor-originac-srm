...
export class CapitalMarketIngestionService {
  ...
  private async runDataset(
    ...
    try {
      ...
      const resourcesToProcess: Array<{ resource: CvmResource; checkpoint?: ResourceCheckpoint }> = [];
      ...
      for (const resource of resources) {
        ...
        if (shouldSkipCapitalMarketResource({ ...options, resource, checkpoint })) {
          ...
          resourcesToProcess.push({ resource, checkpoint });
        }
      }
      ...
      for (let index = 0; index < resourcesToProcess.length; index += 1) {
        const { resource, checkpoint } = resourcesToProcess[index];
