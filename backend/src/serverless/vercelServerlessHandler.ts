...
const normalizePathname = (pathname: string) => pathname.replace(/^\/api(?=\/|$)/, '') || '/';
...
async function runCaptureRuntime(req: IncomingMessage, res: ServerResponse, triggerType: 'cron' | 'manual', options: Required<HandlerOptions>) {
  if (!isAuthorizedRuntime(req)) {
  ...
  const scopeType = requestedCompanyId ? 'company' : requestedSourceId ? 'source' : 'global';
...
async function originationRuntime(req: IncomingMessage, res: ServerResponse) {
  const pathname = normalizePathname(parseUrl(req).pathname);
  const mod = await import('../modules/originationOperatingSystem.js');
  const payload = (() => {
...
export function createVercelServerlessHandler(options: HandlerOptions = {}) {
  const resolvedOptions: Required<HandlerOptions> = {
    auditVersion: options.auditVersion ?? 'serverless_capture_runtime_v1',
  };
  ...
  return async function handler(req: IncomingMessage, res: ServerResponse) {
    ...
    if (normalizedPathname === '/data-capture/cron/run') {
      await runCaptureRuntime(req, res, 'cron', resolvedOptions);
      return;
    }

    if (normalizedPathname === '/data-capture/run') {
      await runCaptureRuntime(req, res, 'manual', resolvedOptions);
      return;
    }

    if (normalizedPathname.startsWith('/origination/')) {
