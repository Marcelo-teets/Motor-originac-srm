...
export const agentDefinitions: AgentDefinition[] = [
  ...
  // ABA/Paper Clip/ADM executam em memória (sem fila durável nem audit trail
  // persistido) — verdade operacional (P0): não podem se declarar 'real'.
  { name: 'aba_build_agent', objective: 'Habilitar Auto Build Agents (ABA) para automelhoria contínua do produto e da operação comercial.', inputs: ['dashboard_summary', 'agents_health', 'pipeline'], outputs: ['aba_commands', 'aba_improvement_backlog'], writes: ['aba_command_runs', 'aba_improvement_backlog'], successCriteria: ['backlog priorizado', 'comandos executáveis'], fallback: 'gerar sugestões estáticas', status: 'partial' },
  { name: 'paper_clip_agent', objective: 'Executar comandos de construção rápida de playbooks e tarefas operacionais orientadas à originação.', inputs: ['aba_commands', 'ranking_v2', 'pipeline'], outputs: ['playbook_actions', 'tasks'], writes: ['tasks', 'activities'], successCriteria: ['ações criadas', 'tarefas atribuídas'], fallback: 'criar template operacional', status: 'partial' },
  { name: 'adm_agent', objective: 'Coordenar decisões administrativas e de governança operacional sobre execução comercial.', inputs: ['aba_commands', 'source_health', 'pipeline'], outputs: ['governance_actions', 'owners_queue'], writes: ['tasks', 'activities'], successCriteria: ['comando roteado', 'responsável definido'], fallback: 'roteamento manual', status: 'partial' },
...
// Resumo de saúde derivado das definições declaradas — nunca contagens
// hard-coded (verdade operacional, P0).
export const agentHealthSummary = () => {
  const counts = { healthy: 0, degraded: 0, mocked: 0 };
  for (const agent of agentDefinitions) {
    if (agent.status === 'real') counts.healthy += 1;
    else if (agent.status === 'partial') counts.degraded += 1;
    else counts.mocked += 1;
  }
  return { ...counts, total: agentDefinitions.length, derivedFrom: 'agent_definitions' };
};
