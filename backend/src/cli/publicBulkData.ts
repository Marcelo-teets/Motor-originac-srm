...
const result = await new PublicBulkIngestionService().run({
  datasets,
...
if (args.includes('--require-scan') && result.totals.rowsScanned <= 0) {
  console.error('Public bulk ingestion completed without scanning source rows.');
  process.exitCode = 1;
}
if (args.includes('--require-matches') && result.totals.recordsMatched <= 0) {
