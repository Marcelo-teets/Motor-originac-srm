...
const rest = async (ctx:Context,path:string,init:RequestInit={},expected:number[]=[200]) => {
  const response=await fetch(`${ctx.baseUrl}/rest/v1/${path}`,{...init,headers:{apikey:ctx.anonKey,Authorization:`Bearer ${ctx.token}`,'Content-Type':'application/json',...(init.headers??{})}});
  const raw=await response.text(); const parsed=raw?(()=>{try{return JSON.parse(raw);}catch{return raw;}})():null;
...
export default async function dcmOutreachLearningHandler(req: IncomingMessage, res: ServerResponse) {
  try {
    const ctx=await authenticate(req); const method=(req.method??'GET').toUpperCase();
    if(method==='GET') {
      ...
      write(res,200,{status:'real',generatedAt:new Date().toISOString(),data:{feedback,rules}}); return;
    }
    if(method!=='POST'){write(res,405,{status:'partial',error:'Method not allowed.'});return;}
    const input=await body(req); const action=text(input.action); const id=text(input.id);
    ...
    if(action==='review_feedback') {
      ...
      const data=await rest(ctx,`dcm_outreach_feedback?id=eq.${encodeURIComponent(id)}&select=*`,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify({learned_rules:learnedRules,change_summary:text(input.changeSummary,input.change_summary)||null,feedback_status:'reviewed',reviewed_by:ctx.user.id,reviewed_at:now})});
      write(res,200,{status:'real',generatedAt:now,data}); return;
    }
    if(action==='set_rule_status') {
      ...
      const data=await rest(ctx,`dcm_outreach_learning_rules?id=eq.${encodeURIComponent(id)}&select=*`,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify({status,reviewed_by:ctx.user.id,reviewed_at:now})});
      write(res,200,{status:'real',generatedAt:now,data}); return;
    }
    ...
    console.error('[dcm-outreach-learning]',error); write(res,statusCode,{status:'partial',generatedAt:new Date().toISOString(),error:error instanceof Error?error.message:String(error)});
  }
}
