...
const executeAction = async (action:string, context:JsonObject) => {
  ...
  if(action==='recompute_company') { if(!UUID_PATTERN.test(companyId)) throw Object.assign(new Error('companyId is required.'),{statusCode:422}); const snapshot=await service.recomputeDerivedData(companyId); await client.rpc('refresh_ranking_v2',{}); return {companyId,qualifications:snapshot.qualifications.length,patterns:snapshot.patterns.length,scores:snapshot.scoreSnapshots.length,leadScores:snapshot.leadScoreSnapshots.length}; }
  ...
  if(action==='recompute_derived_all') { const snapshot=await service.recomputeDerivedData(); await client.rpc('refresh_ranking_v2',{}); return {qualifications:snapshot.qualifications.length,patterns:snapshot.patterns.length,scores:snapshot.scoreSnapshots.length,leadScores:snapshot.leadScoreSnapshots.length}; }
