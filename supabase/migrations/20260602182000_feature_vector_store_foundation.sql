-- Supabase preview/apply mirror for db/migrations/024_feature_vector_store_foundation.sql
...
begin
  insert into public.company_feature_snapshots (
    company_id, feature_key, feature_value_numeric, feature_value_text, feature_value_boolean,
    feature_value_json, confidence, evidence_url, source_code, source_document_id,
    monitoring_output_id, run_id, payload_hash, observed_at, captured_at
  ) values (
    p_company_id, p_feature_key, p_feature_value_numeric, p_feature_value_text, p_feature_value_boolean,
    coalesce(p_feature_value_json, '{}'::jsonb), coalesce(p_confidence, 0.50), p_evidence_url,
    p_source_code, p_source_document_id, p_monitoring_output_id, p_run_id, p_payload_hash,
    coalesce(p_observed_at, now()), captured_timestamp
  )
  ...
  insert into public.company_feature_current (
    company_id, feature_key, feature_value_numeric, feature_value_text, feature_value_boolean,
    feature_value_json, confidence, evidence_url, source_code, source_document_id,
    monitoring_output_id, run_id, payload_hash, observed_at, captured_at, updated_at
  ) values (
    p_company_id, p_feature_key, p_feature_value_numeric, p_feature_value_text, p_feature_value_boolean,
    coalesce(p_feature_value_json, '{}'::jsonb), coalesce(p_confidence, 0.50), p_evidence_url,
    p_source_code, p_source_document_id, p_monitoring_output_id, p_run_id, p_payload_hash,
    coalesce(p_observed_at, now()), captured_timestamp, now()
  )
...
declare
  ...
  content_text text;
begin
  select sd.*, coalesce(sc.metadata->>'code', sc.id::text, 'unknown') as source_code
  into doc
  from public.source_documents sd
  left join public.source_catalog sc on sc.id::text = sd.source_id::text
  ...
  insert into public.copilot_context_documents (
    company_id, source_document_id, source_code, context_type, title, content, summary,
    metadata, evidence_url, payload_hash, confidence, observed_at, captured_at, embedding_status
  ) values (
    doc.company_id,
...
insert into public.feature_definitions (
  feature_key, feature_name, feature_group, value_type, description,
  source_policy, calculation_policy, freshness_sla_hours
) values
