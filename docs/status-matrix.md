...
| Deployment | Atenção operacional | O primeiro deploy de produção após o merge da PR #131 falhou antes do build em `build-container-init`, com erro Vercel `sts_credentials_fetch_failed`. O preview do mesmo código estava `READY`; este commit registra o incidente e dispara nova tentativa sem alterar runtime. |
